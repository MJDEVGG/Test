...
# ========== GAMES CONFIGURATION ==========
BOT_TOKEN = "7991832885:AAFERva-_KSoet1x_mvSspn6GA4-1wELMng"
...
OWNER_ID = [8112555317, 6453151474]
BOT_TOKEN = "7991832885:AAFERva-_KSoet1x_mvSspn6GA4-1wELMng"

# ========== IMAGE SETTINGS ==========
WELCOME_IMAGE_URL = "https://i.postimg.cc/nV3Lv1mJ/Picsart-25-11-08-21-26-18-912.jpg"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === ADDED: enforce private-only start ===
    try:
    ...
    if chat_type and chat_type != "private":
        await update.message.reply_text("This command can used inside bot only.")
        return
    # === end added check ===
    ...
    text = (
        f"══════════════════\n"
        f"𝗕𝗢𝗦𝗦 𝗥 𝗖𝗣𝗠 𝟭 & 𝟮 𝗧𝗢𝗢𝗟\n"
        f"══════════════════\n\n"
        f"▬▬▬▬▬▬▬▬▬▬▬▬▬\n"
        f"𝗔𝗖𝗖𝗢𝗨𝗡𝗧 𝗜𝗡𝗙𝗢𝗥𝗠𝗔𝗧𝗜ON\n"
        f"▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n"
        f"🆔 𝗧𝗲𝗹𝗲𝗴𝗿𝗮𝗺 𝗜𝗗: {uid}\n"
        f"🎰 𝗦𝗹𝗼𝘁𝘀: {USERS[uid]['slots']}\n"
        f"💰 𝗕𝗮𝗹𝗮𝗻𝗰𝗲: {format_balance(USERS[uid]['balance'])}\n"
        f"👑 𝗦𝘁𝗮𝘁𝘂𝘀: {vip_status}\n"
        f"🚫 𝗕𝗹𝗼𝗰𝗸𝗲𝗱: {blocked_status}\n"
        f"▬▬▬▬▬▬▬▬▬▬▬▬▬\n\n"
        f"𝗦𝗨𝗣𝗣𝗢𝗥𝗧 & 𝗖𝗢𝗠𝗠𝗨𝗡𝗜𝗧𝗬\n\n"
        f"DEVELOPER: @bossrtoolls\n"
        f"OWNER: @bossrtoolls\n"
        f"GROUP: @bossrtools2\n"
        f"CHANNEL: @bossrtools\n"
     ) 
    try:
...
async def handle_all_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "king_rank":
        ...
        keyboard = [
            [InlineKeyboardButton("💬 Contact @bossrtoolls", url="https://t.me/bossrtoolls")],
            [InlineKeyboardButton("🔙 Back", callback_data="back_to_main")],
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    # Mining buttons
        ...
        if not is_vip(uid):
            await safe_edit_message(
                query,
                "❌ You don't have any available slots! Contact @bossrtoolls to buy slots."
            )
            return
        ...
        if accounts_count >= available_slots:
            await safe_edit_message(
                ...
                f"💡 Contact @bossrtoolls to buy more slots"
            )
            return
...
# ========== ADMIN: NEW NON-DESTRUCTIVE COMMANDS ADDED ==========
# These are added in addition to your original /block (which is preserved).
async def block_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/block_user <user_id>  - non-destructive addition (keeps original /block)"""
    user = update.effective_user
    if user.id != OWNER_ID:
        await update.message.reply_text("🚫 You are not authorized to use this command.")
        return
    ...
    save_users()
    await update.message.reply_text(f"✅ User {target} has been blocked.")
    try:
...
async def unblock_user_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/unblock_user <user_id>  - non-destructive addition"""
    user = update.effective_user
    if user.id != OWNER_ID:
        await update.message.reply_text("🚫 You are not authorized to use this command.")
        return
    ...
    save_users()
    await update.message.reply_text(f"✅ User {target} has been unblocked.")
    try:
        await context.bot.send_message(chat_id=target, text="✅ You have been unblocked and can now use the bot again.")
    except Exception as e:
        print(f"Failed to notify user: {e}")

async def unlimited_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/unlimited <user_id>  - sets a very large balance (treated as unlimited)"""
    user = update.effective_user
    if user.id != OWNER_ID:
        await update.message.reply_text("🚫 You are not authorized to use this command.")
        return
    ...
    if target not in USERS:
        await update.message.reply_text("❌ User not found in database.")
        return
    # sentinel large number for unlimited
    USERS[target]["balance"] = 9999999999
    save_users()
    await update.message.reply_text(f"♾️ User {target} now has Unlimited balance.")
    try:
        await context.bot.send_message(chat_id=target, text="💰 Your balance is now ♾️ Unlimited!")
    except Exception as e:
        print(f"Failed to notify user: {e}")

async def limited_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/limited <user_id>  - resets balance back to 0 (non-destructive)"""
    user = update.effective_user
    if user.id != OWNER_ID:
        await update.message.reply_text("🚫 You are not authorized to use this command.")
        return
    ...
    save_users()
    await update.message.reply_text(f"💰 User {target}'s balance reset to normal (0).")
    try:
        await context.bot.send_message(chat_id=target, text="💰 Your balance has been restored to normal (0).")
    except Exception as e:
        print(f"Failed to notify user: {e}")
...
# preserve existing handlers and also add the new non-destructive admin commands:
...
app.add_handler(CommandHandler("limited", limited_command))
